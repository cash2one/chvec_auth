...
time=$8
	...
	#/root/bin/ffmpeg -i /var/www/chvec_auth/video/156.mp4 -vf "drawtext=fontsize=45:fontcolor=#C9C9C9@0.9: fontfile=/usr/share/fonts/winfonts/AgencyFB.ttf: text=China:y=h-text_h-line_h:x=w-2.2*text_w,drawtext=fontsize=45:fontcolor=red@0.5: fontfile=/usr/share/fonts/winfonts/AgencyFB.ttf: text=VEC:y=h-text_h-line_h:x=w-1.6*text_w" -y -qscale 3 /var/www/chvec_auth/text2.mpg
	...
	/root/bin/ffmpeg -i $video_path -qscale 3 /var/www/chvec_auth/tmp/$ID.mpg 2>&1 | tee /var/www/chvec_auth/progress/$num.txt
