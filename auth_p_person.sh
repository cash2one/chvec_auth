...
subtitle=$9
	...
	#加竖版字幕版
	if [ $subtitle ]; then
	/root/bin/ffmpeg -threads 2 -i $video_path -vf "drawtext=fontsize=45:fontcolor=#C9C9C9@0.9: fontfile=/usr/share/fonts/winfonts/AgencyFB.ttf: text=China:y=h-text_h-line_h:x=w-2.2*text_w: enable=lt(mod(t\,$period)\,$time),drawtext=fontsize=45:fontcolor=red@0.5: fontfile=/usr/share/fonts/winfonts/AgencyFB.ttf: text=VEC:y=h-text_h-line_h:x=w-1.6*text_w: enable=lt(mod(t\,$period)\,$time),drawtext=fontsize=25:fontfile=/usr/share/fonts/winfonts/SIMHEI.TTF:textfile=credit.txt:fontcolor=#C9C9C9@0.8:y='if(gte(t\,6)\,h-((t-6)*80)\,NAN)'" -y -qscale 3 /var/www/chvec_auth/tmp/$ID.mpg 2>&1 | tee /var/www/chvec_auth/progress/$num.txt
	else
	#处理视频添加ChinaVEC的文字logo，位置右下角自适应视频画面，常在#	
	/root/bin/ffmpeg -threads 2 -i $video_path -vf "drawtext=fontsize=45:fontcolor=#C9C9C9@0.9: fontfile=/usr/share/fonts/winfonts/AgencyFB.ttf: text=China:y=h-text_h-line_h:x=w-2.2*text_w: enable=lt(mod(t\,$period)\,$time),drawtext=fontsize=45:fontcolor=red@0.5: fontfile=/usr/share/fonts/winfonts/AgencyFB.ttf: text=VEC:y=h-text_h-line_h:x=w-1.6*text_w: enable=lt(mod(t\,$period)\,$time)" -y -qscale 3 /var/www/chvec_auth/tmp/$ID.mpg 2>&1 | tee /var/www/chvec_auth/progress/$num.txt
	fi
	...
	/root/bin/ffmpeg -i concat:"/var/www/chvec_auth/tmp/$num.mpg|/var/www/chvec_auth/tmp/$ID.mpg" -c:v copy -c:a copy /var/www/chvec_auth/video_auth/$num.$suf
