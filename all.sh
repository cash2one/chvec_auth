...
time=$8
	...
	#/root/bin/ffmpeg -i /var/www/chvec_auth/video/6.mp4 -vf "drawtext=fontsize=25:fontfile=/usr/share/fonts/winfonts/SIMHEI.TTF:text=授权码：CP10323132144         有效期：2014-10-08   :fontcolor=white@0.5:x='if(gte(t\,2)\,((t-2)*80)-w\,NAN)':y=line_h" -y /var/www/chvec_auth/test1.mpg

	#/root/bin/ffmpeg -i /var/www/chvec_auth/video/156.mp4 -vf "drawtext=fontsize=45:fontcolor=#C9C9C9@0.9: fontfile=/usr/share/fonts/winfonts/AgencyFB.ttf: text=China:y=h-text_h-line_h:x=w-2.2*text_w: enable=lt(mod(t\,10)\,5),drawtext=fontsize=45:fontcolor=red@0.5: fontfile=/usr/share/fonts/winfonts/AgencyFB.ttf: text=VEC:y=h-text_h-line_h:x=w-1.6*text_w: enable=lt(mod(t\,10)\,5)" -y -qscale 3 /var/www/chvec_auth/text2.mpg
