...
$(function(){
	...
	console.log(value);
	var playscene = $("#playscene").val();
	console.log(playscene);
	if(value=='P'){
	$('#playscene').css("display","block");
...
OPTION;
	...
	<form id="authForm" name="authForm" action="do.php" method="post" enctype="multipart/form-data">
		...
		<p><span style="float:left;margin:0;height:35px;line-height:35px;">选择授权类型：</span>
			...
			<select id="playscene" class="playscene" type="text"  name="playscene" style="display:none;margin-left:10px;width:30%;float:left;">
		...
		<p id="mark" > 
		<input name="xianshi" id="xianshi" class="xianshi" type="checkbox" disabled="disabled" checked="checked"/>
		<span>版权角标</span>
		</p>
