...
time=$8
	...
	/root/bin/ffmpeg -threads 2 -i $video_path -vf "drawtext=fontsize=45:fontcolor=#C9C9C9@0.9: fontfile=/usr/share/fonts/winfonts/AgencyFB.ttf: text=China:y=h-text_h-line_h:x=w-2.2*text_w,drawtext=fontsize=45:fontcolor=red@0.5: fontfile=/usr/share/fonts/winfonts/AgencyFB.ttf: text=VEC:y=h-text_h-line_h:x=w-1.6*text_w,drawtext=fontsize=25:fontfile=/usr/share/fonts/winfonts/SIMHEI.TTF:text=有效期至'$date'                        授权'$num':fontcolor=#C9C9C9@0.8:x='if(gte(t\,2)\,((t-2)*80)-w\,NAN)':y=line_h,drawtext=fontsize=25:fontfile=/usr/share/fonts/winfonts/SIMHEI.TTF:text=有效期至'$date'                        授权'$num':fontcolor=#C9C9C9@0.8:x='if(gte(t\,60)\,((t-60)*80)-w\,NAN)':y=line_h" -y -qscale 3 /var/www/chvec_auth/tmp/$ID.mpg
